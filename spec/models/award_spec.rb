...
RSpec.describe Award, type: :model do
  ...
  describe 'validations' do
    ...
    it 'is valid with an attached image' do
      award.image.attach(io: File.open(Rails.root.join('spec/fixtures/1x1.png')), filename: '1x1.png', content_type: 'image/png')
      expect(award).to be_valid
    end

    it 'is invalid with wrong image type' do
      award.image.attach(io: File.open(Rails.root.join('spec/fixtures/1x1.svg')), filename: '1x1.svg', content_type: 'image/svg')
